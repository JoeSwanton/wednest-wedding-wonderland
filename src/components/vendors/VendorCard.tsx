...
const VendorCard = ({ vendor }: VendorCardProps) => {
  ...
  return (
    <Card className="overflow-hidden bg-white border border-theme-beige rounded-lg hover:shadow-md transition-shadow duration-300">
      {/* Image and badges */}
      <div className="relative h-48 bg-muted overflow-hidden">
        <div className="absolute top-2 left-2 bg-theme-blue text-white px-2 py-1 rounded text-xs font-medium z-10">
          {vendor.type}
        </div>
        ...
        {vendor.rating > 4.5 && (
          <div className="absolute bottom-2 left-2 bg-blue-600 text-white px-2 py-0.5 rounded-sm text-xs font-medium z-10 flex items-center">
            Top Rated
          </div>
        )}
        {vendor.availability.includes("Limited") && (
          <div className="absolute top-2 right-12 bg-red-500 text-white px-2 py-0.5 rounded-sm text-xs font-medium z-10">
      ...
      <div className="p-3">
        ...
        {vendor.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-2">
            {vendor.tags.slice(0, 2).map((tag, index) => (
              <span 
                key={index}
                className="inline-block bg-theme-cream text-theme-gray-dark rounded px-1.5 py-0.5 text-xs"
              >
                {tag}
              </span>
            ))}
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          <Link to={`/vendors/${vendor.id}`}>
            <Button 
              className="bg-theme-blue hover:bg-theme-blue-dark text-white text-xs px-3 h-8"
            >
              Check Availability
            </Button>
          </Link>
        ...
        {vendor.availability.includes("contacted") && (
          <div className="mt-2 text-xs text-theme-gray-dark">
