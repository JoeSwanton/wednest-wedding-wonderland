...
const BusinessDescriptionStep = ({
  ...
  return <div className="space-y-6">
      <div className="space-y-2">
        <h2 className="text-2xl font-serif text-wednest-brown">Business Profile</h2>
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <Textarea id="bio" name="bio" value={localFormData.bio} onChange={handleChange} placeholder="Describe your business, experience, and what makes you unique..." rows={6} required />
        ...
        <div className="space-y-2">
          ...
          <Input id="tagline" name="tagline" value={localFormData.tagline} onChange={handleChange} placeholder="E.g. 'Capturing timeless moments for your special day'" maxLength={100} />
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            <Input id="specialtyInput" value={specialtyInput} onChange={e => setSpecialtyInput(e.target.value)} placeholder="E.g. Eco-friendly, Destination weddings, Luxury" onKeyDown={e => {
            if (e.key === 'Enter') {
              e.preventDefault();
              handleAddSpecialty();
            }
          }} />
            <Button type="button" variant="secondary" onClick={handleAddSpecialty}>
              Add
            </Button>
          </div>
          
          <div className="flex flex-wrap gap-2 mt-3">
            {localFormData.specialties.length === 0 ? <p className="text-sm text-wednest-brown-light italic">
                No specialties added yet. Add what makes your business special.
              </p> : localFormData.specialties.map((specialty, index) => <Badge key={index} variant="outline" className="px-2 py-1 bg-wednest-beige/20">
                  {specialty}
                  <button type="button" onClick={() => handleRemoveSpecialty(specialty)} className="ml-1 text-wednest-brown-light hover:text-wednest-brown">
                    <X size={14} />
                  </button>
                </Badge>)}
          </div>
      ...
      <div className="pt-4 flex justify-between">
        ...
        <Button type="button" onClick={handleSubmit} className="bg-wednest-sage hover:bg-wednest-sage-dark">
          Continue
        </Button>
      </div>
    </div>;
